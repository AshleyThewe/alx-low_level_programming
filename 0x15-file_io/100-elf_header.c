 ...
 * @e_ident: A pointer to array containing the ELF file magic numbers.
 *
 * Description: If the files are not an ELF file - exit the code.
 */
void check_elf(unsigned char *e_ident)
{
	int index;
 ...
 *
 * Description: Magic number or numbers  is or are indeed  separated by spaces.
 */
void print_magic(unsigned char *e_ident)
{
	int index;
 ...
 *
 * Description: If the file cannot be closed - exit the code.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
 ...
 * Return: 0 on success.
 *
 * Description: If the file is not an ELF File or not or
